...
public final class ColorWidgets {
    ...
    static RenderableRectangle withMouseCursor(CursorType cursor) {
        return (graphics, left, top, width, height, mouseX, mouseY, ignoredD) -> {
            if (ScreenRectangleUtils.containsPoint(left, top, width, height, mouseX, mouseY) &&
                graphics.containsPointInScissor(left, top)) {
    ...
    static WidgetRenderables thumbWidget(IntIntMutablePair position, IntSupplier argb) {
        ...
        return new WidgetRenderables(
                base,
                base.then(withMouseCursor(CursorTypes.NOT_ALLOWED)),
                highlighted.then(withMouseCursor(CursorTypes.POINTING_HAND)),
                base.then(withMouseCursor(CursorTypes.NOT_ALLOWED))
        );
    }
    ...
    static WidgetRenderables sliderBackground(IntIntMutablePair position, RenderableRectangle background) {
        ...
        RenderableRectangle checkeredBackground = checkerboard(position).then(background).crop(1);
